  ...
  return {
    package: rawKey => {
      ...
      for (const section in packagedCrate[key].template) {
        if (Object.prototype.hasOwnProperty.call(packagedCrate[key].template, section)) {
          const sectionElem =
    ...
    showTags: rawKey => {
      ...
      const gatherTaggers = (taggedByArray, entry) => {
        ...
        if (length > 0) {
          taggedByArray.push({project: entry, times: length})
        }
...
const createElem = elemObject => {
  ...
  if (elemObject.elem.text) {
    elem.innerHTML = elemObject.elem.text
  }
