...
const createProjectObject = (acc, cur) => {
  ...
  acc[cur.project].time += cur.time
  acc[cur.project].entries += 1
  return acc
}
...
const getDaysToCuttoff = amount => [...new Array(amount).keys()].map(getDate)
...
const getTotalMinutes = (total, day) => {
  return day ? total + day.reduce((ltotal, log) => ltotal + log.time, 0) : total
}
